...
import { Share2, Heart, ChevronRight, Scale, Check, Loader2, Plus } from "lucide-react";
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
                <button
                    type="button"
                    onClick={onAddToList}
                    className={`inline-flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-xs font-semibold shadow-sm transition active:scale-95 ${isInList
                            ? "border-emerald-600 bg-emerald-600 text-white hover:bg-emerald-700"
                            : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                        }`}
                >
                    {isInList ? <Check className="h-3.5 w-3.5 text-white" /> : <Plus className="h-3.5 w-3.5 text-slate-500" />}
                    <span>{isInList ? "Added to List" : "Add to University List"}</span>
                </button>
                ...
                >
                    {isCompared ? <Check className="h-3.5 w-3.5 text-white" /> : <Scale className="h-3.5 w-3.5 text-slate-500" />}
                ...
                <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-xs font-semibold shadow-sm transition active:scale-95 disabled:opacity-50 ${isFavorite ? "border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                        }`}
                >
                    {loading ? (
                        <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-400" />
                    ) : (
                        <Heart className={`h-3.5 w-3.5 transition-colors ${isFavorite ? "fill-rose-500 text-rose-500" : "text-slate-400"}`} />
                    )}
